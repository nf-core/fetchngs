...
**nf-core/fetchngs** is a bioinformatics pipeline to fetch metadata and raw FastQ files from public databases. At present, the pipeline supports [SRA](https://www.ncbi.nlm.nih.gov/sra) / [ENA](https://www.ebi.ac.uk/ena/browser/home) / [DDBJ](https://www.ddbj.nig.ac.jp/index-e.html) / [GEO](https://www.ncbi.nlm.nih.gov/geo/) / [Synapse](https://www.synapse.org/#) ids (see [usage docs](https://nf-co.re/fetchngs/usage#introduction)).
...
1. Resolve database ids back to appropriate experiment-level ids and to be compatible with the [ENA API](https://ena-docs.readthedocs.io/en/latest/retrieval/programmatic-access.html), or appropriate database
2. Fetch extensive id metadata including direct download links to FastQ files via ENA API or appropriate database
3. Download FastQ files in parallel and perform `md5sum` check
