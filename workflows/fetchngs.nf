...
workflow FETCHNGS {
    ...
    SRA_TO_SAMPLESHEET.out.samplesheet.map { _meta, samplesheet -> samplesheet }.collectFile(name: 'tmp_samplesheet.csv', newLine: true, keepHeader: true, sort: { file -> file.baseName }).map { file -> file.text.tokenize('\n').join('\n') }.collectFile(name: 'samplesheet.csv', storeDir: "${outdir}/samplesheet").set { ch_samplesheet }

    SRA_TO_SAMPLESHEET.out.mappings.map { _meta, mappings -> mappings }.collectFile(name: 'tmp_id_mappings.csv', newLine: true, keepHeader: true, sort: { file -> file.baseName }).map { file -> file.text.tokenize('\n').join('\n') }.collectFile(name: 'id_mappings.csv', storeDir: "${outdir}/samplesheet").set { ch_mappings }
    ...
    versions        = ch_collated_versions.unique()
}
