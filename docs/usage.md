...
The final sample information for all identifiers is obtained from the ENA which provides direct download links for FastQ files as well as their associated md5 sums. If download links exist, the files will be downloaded in parallel by FTP. Otherwise they are downloaded using sra-tools. To download protected dbGAP data using sra-tools, a JWT cart file can be specified with `--dbgap_key /path/to/cart.jwt`. 
