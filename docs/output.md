...
## SRA / ENA / DDBJ / GEO - Pipeline Overview
...
## Synapse - Pipeline Overview

The pipeline is built using [Nextflow](https://www.nextflow.io/) and processes data using the following steps:

* [FastQ download](#fastq-Download) - Download FastQ files via Synapse ids
* [Pipeline information](#pipeline-Information) - Report metrics generated during the workflow execution
...
* `metadata/`
    ...
    * `*.list.txt`: Original output of the `synapse list` command, containing the SynapseIDs, file version numbers, file names, and other file-specific data of the Synapse directory ID provided.
